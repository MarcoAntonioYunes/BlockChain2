...
export class AppComponent implements AfterViewInit {
  ...
  numeroboletos = new FormGroup({
    dircant: new FormControl('')
    });

    boletocine = new FormGroup({
  ...
  async Balance(): Promise<void> {
    const addressDapp =  this.numeroboletos.get('dircant')?.value;
  ...
  async transferir(): Promise<void> {
    const address1 =  this.boletocine.get('recep')?.value;
    const numsgc =  this.boletocine.get('nump')?.value;
